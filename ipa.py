  ...
  try:
    ...
    for lang in langs:
      url = "https://od-api.oxforddictionaries.com:443/api/v2/entries/" + \
          lang + "/" + word.lower()
      reply_json = requests.get(
          url, headers={"app_id": app_id, "app_key": app_key}).json()

      reply_str = json.dumps(reply_json, indent=2)
      reply_dict = json.loads(reply_str)
      spellings.append(reply_dict['results'][0]['lexicalEntries']
                      [0]['entries'][0]['pronunciations'][1]['phoneticSpelling'])
